...
def meanerror(A, K, H_list, corner_list):
    ...
    for i in range(len(H_list)):
        ...
        img_points, _ = cv2.projectPoints(
            world, lr1r2t[:, 0:3], lr1r2t[:, 3], A, K)
        img_points = np.array(img_points)
        errors = np.linalg.norm(np.subtract(
            img_points[:, 0, :], corner_list[i][:, 0, :]), axis=1)
...
for img in images:
    ...
    for k in [0, 8, 53, 45]:
        ...
        cv2.circle(img, (int(corners[k][0][0]), int(
            corners[k][0][1])), 10, (255, 0, 0), 2)

    camera = np.float32(camera)

    world = np.float32([(21.5, 21.5), (21.5*9, 21.5),
...
initial = param(A, 0, 0)
res = least_squares(function, x0=np.squeeze(initial),
...
for ind in range(len(images)):
    ...
    for k in range(54):
        cv2.circle(undist, (int(corners2[0, k]), int(
            corners2[1, k])), 20, (255, 0, 0), 1)

        cv2.circle(undist, (int(corners[k][0][0]), int(
            corners[k][0][1])), 20, (0, 0, 255), 1)
